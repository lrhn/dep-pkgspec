...
An example `packages.map` file generated by the pub tool could be:
...
- The proposed default name is `packages.map`.
...
in the specification file `file:///home/somebody/dart/project/smarty/packages.map`, then the base path of the package `unittest` is `file:///home/somebody/dart/packages/unittest-0.9.9/lib/`. The remaining path "unittest.dart" is resolved against this, getting `file:///home/somebody/dart/packages/unittest-0.9.9/lib/unittest.dart`.
...
1. Look for a `packages.map` file next to the program entry point (which can then not be given using a package: URI). For example running an application like:  
    `dart http://example.com/smarty/main.dart`  
will cause the `dart` stand-alone VM to check for the existence of `http://example.com/smarty/packages.map`, and if that URI returns a file, use the content for resolving package URIs in the application.

2. If a tool does not find a `packages.map` file in the previous step, it should look for a `packages` directory next to the entry point. If the entry point is a `file:` URI this means checking if a `packages` directory exists, in all other cases, the directory is assumed to exist.
...
As part of the implementation of this proposal, the "pub" tool should be changed to allow writing a `packages.map` file where it currently creates a "package" directory in a package's root directory.
Pub should avoid automatically creating duplicate `packages.map` files in other locations.
...
There should be a Dart package for reading and writing `packages.map` files, converting to and from `Map<String, Uri>`. This can be used by all Dart based tools that need to read or write the package resolution file.
